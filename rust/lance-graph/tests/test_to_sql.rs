...
async fn test_to_sql_simple_node_scan() {
    ...
    assert!(sql.to_uppercase().contains("SELECT"), "SQL should contain SELECT");
    assert!(sql.to_lowercase().contains("person"), "SQL should reference person table");
...
async fn test_to_sql_with_filter() {
    ...
    assert!(sql.contains("WHERE") || sql.contains("FILTER"), "SQL should contain WHERE clause");
...
async fn test_to_sql_with_relationship() {
    ...
    assert!(sql_lower.contains("company"), "SQL should reference company");
...
async fn test_to_sql_with_alias() {
    ...
    let query = CypherQuery::new ("MATCH (p:Person) RETURN p.name AS person_name, p.age AS person_age")
        .unwrap()
        .with_config(config);
    ...
    assert!(sql.contains("AS") || sql.contains("as"), "SQL should contain AS for aliases");
...
async fn test_to_sql_complex_query() {
    ...
    assert!(sql.contains("SELECT"), "SQL should contain SELECT");
    assert!(sql.contains("JOIN") || sql.contains("join"), "SQL should contain JOIN");
    assert!(sql.contains("WHERE") || sql.contains("where"), "SQL should contain WHERE");
    assert!(sql.contains("ORDER BY") || sql.contains("order by"), "SQL should contain ORDER BY");
    assert!(sql.contains("LIMIT") || sql.contains("limit"), "SQL should contain LIMIT");
...
async fn test_to_sql_empty_datasets() {
    ...
    assert!(
        result.unwrap_err().to_string().contains("No input datasets"),
